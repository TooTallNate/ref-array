...
module.exports = function Array (_type, _length) {
  ...
  function ArrayType (data, length) {
    ...
    if (0 === arguments.length) {
      // new IntArray()
      // use the "defaultLength" if provided, otherwise throw an Error
      if (defaultLength > 0) {
      ...
      // new IntArray([ 1, 2, 3, 4, 5 ])
      this.length = null == length ? data.length : length
      this.buffer = new Buffer(this.length * item_size)
      ...
      this.buffer = data;
      this.length = null == length ? data.length / item_size : length
    }
  }
  ...
  ArrayType.size = ArrayType.BYTES_PER_ELEMENT * defaultLength
